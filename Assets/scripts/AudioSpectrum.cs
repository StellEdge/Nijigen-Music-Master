...
public class AudioSpectrum : MonoBehaviour
{
    ...
    public BandType bandType = BandType.TwentySixBand;
    ...
    void Update()
    {
        ...
        //Update Curve
        int widthPixels = (int)(Screen.width * width *0.1f);
        int heightPixels = (int)(Screen.height * height * 0.1f);
        lineRenderer.SetVertexCount(levels.Length);

        Vector3 cur_pos = this.transform.localPosition;
        for (var bi = 0; bi < levels.Length; bi++)
        {
            lineRenderer.SetPosition(bi, new Vector3(cur_pos[0] + 1.0f / levels.Length * bi * widthPixels, cur_pos[1]+ levels[bi]* heightPixels, cur_pos[2]) );
        }
    }
}